...
and loads the saved songs into customized playlists based on six features.

Requirments: spotipy library installed on device, Spotify account and dashboard app created
...
## CODE PROVIDED BY SPOTIPY ON GITHUB TO GO THROUGH AUTHORIZATION FLOW ##
...
USERNAME = '1255650943' #Ariel's user profile, 
...
#
#this section is for playlists of reccomendations, but the input is different than other input data
#so this section is still being worked on 
#if inp in ['y', 'Y','yes','Yes','YES']:
#    
#    #recommendations based on clustered tracks
#    playRand = sp.recommendations(seed_tracks = playSelect, limit = 20)
#    #send playlist back to Spotify server, is now available on on user's profile
...
#        results = sp.user_playlist_add_tracks(USERNAME, playlistConfirm['id'], playRand)
